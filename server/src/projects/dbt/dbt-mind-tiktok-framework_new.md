# DBT-Mind TikTok Slideshow Framework

## Account Setup
- **Account name:** amy_bpd (personal account, not brand)
- **Bio:** Always include app link with personal framing
- **Positioning:** Person with BPD sharing what helps, not a company promoting an app

---

## Content Mix Strategy

| Content Type | % of Posts | Purpose | App Mention |
|--------------|------------|---------|-------------|
| **Relatable/Emotional** | 50% | Community building, high engagement | Comment-bait on select posts |
| **POV** | 20% | Watch time, shares, explaining BPD to others | Rarely, profile link only |
| **Tips/Hacks** | 30% | Saves, follows, educational value | Natural integration on last slide |

---

# FORMAT 1: RELATABLE/EMOTIONAL

## Purpose
Validation content. Makes viewers feel seen. Drives comments like "this is me" and "I feel so called out."

## Structure (7 slides)

| Slide | Function | Text Style |
|-------|----------|------------|
| 1 | **HOOK** | Curiosity gap, max 12 words |
| 2-4 | **Relatable content** | First-person, hyper-specific experiences |
| 5 | **Emotional landing** | The "and that's why..." or deeper realization |
| 6 | **App Slide** | Connect core emotion + DBT-Mind (free) + specific result |
| 7 | **CTA** | Fixed text: "i track it all on DBT-Mind. it's free." |

## Hook Formulas

### "Nobody Talks About" (Highest performing)
Creates insider knowledge + curiosity gap

**Formula:** `"nobody talks about [hyper-specific BPD experience]"`

**Examples by topic:**
- FP: "nobody talks about how having an FP is basically emotional hostage syndrome"
- Splitting: "nobody talks about how fast you can go from 'soulmate' to 'enemy' in your head"
- Quiet BPD: "nobody talks about the version of BPD where you implode instead of explode"
- Abandonment: "nobody talks about how fear of abandonment makes you abandon people first"
- Identity: "nobody talks about not knowing which version of yourself is the real one"

### "Hyper-Specific Symptom" (High recognition)
Instant "that's ME" reaction

**Formula:** `"[tiny specific behavior that feels too personal to be universal]"`

**Examples by topic:**
- FP: "checking if they've been online but haven't texted you back yet"
- Splitting: "when they use a period instead of an exclamation mark and suddenly they hate you"
- Quiet BPD: "smiling and nodding while your brain is screaming that everyone secretly hates you"
- Abandonment: "testing whether they'll stay by showing them your worst"
- Identity: "changing your entire personality based on who you're with"

### "Call-Out" (Direct address)
Stops scroll by speaking directly to viewer

**Formula:** `"if you [specific behavior], this is for you"`

**Examples by topic:**
- FP: "if your entire mood depends on one person's texting patterns, this is for you"
- Splitting: "if you've ever mentally ended a relationship over a tone of voice, stay"
- Quiet BPD: "if you've ever had a complete breakdown and no one around you noticed, this one"
- Abandonment: "if you've ever sabotaged something good because you were waiting for it to end anyway"
- Identity: "if you've ever copied someone's mannerisms without realizing it, this is for you"

### "Confession" (Vulnerability)
Creates instant emotional connection

**Formula:** `"I [vulnerable admission about BPD behavior]"`

**Examples by topic:**
- FP: "I know exactly how many hours it's been since they last messaged"
- Splitting: "I rewrote our entire history in my head because they didn't text back fast enough"
- Quiet BPD: "I've never told anyone how close I am to falling apart at any given moment"
- Abandonment: "I push people away and then cry when they actually leave"
- Identity: "I don't know if I actually like this or if I just absorbed it from someone else"

## Slide Text Guidelines

### DO:
- First person ("I", "my brain") not second person
- Hyper-specific situations, not general statements
- Internal monologue revealing hidden experience
- Slight dark humor / self-aware tone
- Use therapy speak naturally (splitting, FP, dissociating, masking)
- Contrast structure (external appearance vs internal chaos)

### DON'T:
- Generic statements ("People with BPD experience...")
- Educational tone
- Advice-giving
- Second person ("You feel...")
- Clinical language without personal framing

### Text Examples

❌ Generic: "People with BPD experience fear of abandonment"
✅ Viral: "rereading our entire text thread 6 times looking for signs he's losing interest"

❌ Generic: "BPD causes emotional dysregulation"
✅ Viral: "going from 'best day ever' to 'I want to disappear' because someone's reply felt off"

## App Mention Strategy (Relatable Posts)

**Use on:** Every post (Slide 6 is standardized)
**Placement:** Slide 6 (App Slide). Slide 7 is the fixed CTA.
**Tone:** Personal discovery, not promotion

### Slide 6 App Slide Text Options:

**For splitting posts:**
"when i name it in DBT-Mind (free) -- it finally makes sense"

**For FP/attachment posts:**
"when i log it in DBT-Mind (free) -- i can come back to myself"

**For emotional spiral posts:**
"when i use DBT-Mind (free) -- i stop the spiral before it takes over"

**For quiet BPD posts:**
"when i write it in DBT-Mind (free) -- i know what to do next"

**For abandonment posts:**
"my DBT app breaks this down step by step and it's the only thing that's worked"

### What NOT to say:
❌ "This app is great for this"
❌ "DBT-Mind app really helps! Link in bio"
❌ "Check out this amazing app"

**The difference:** Personal framing + no CTA + benefit tied to emotional content = authentic

---

# FORMAT 2: POV

## Purpose
Immersive experience content. Puts viewer INTO the BPD moment. Drives watch time and shares (people send to partners/friends to explain their experience).

## Structure (4-5 slides)

| Slide | Function | Visual Direction |
|-------|----------|------------------|
| 1 | **POV Hook** | Establish the scenario |
| 2-3 | **The experience** | Show what happens internally |
| 4 | **The contrast/twist** | External vs internal, or escalation |
| 5 (optional) | **Resolution or continuation** | Leave them in the feeling |

## Hook Formula

**Formula:** `"POV: [specific BPD moment in second person]"`

The "POV:" prefix signals immersive content. Viewer expects to be placed INTO a scenario.

### POV Hooks by Topic

**Favorite Person (FP):**
| Hook | Visual Direction |
|------|------------------|
| POV: your FP left you on read 3 hours ago | Woman staring out window, anxious |
| POV: they finally texted back and your whole day is saved | Dramatic shift, dark to light |
| POV: you're watching your FP talk to someone else | Jealousy/longing, Renaissance style |
| POV: you're trying to act normal while analyzing their every word | Split composition: calm exterior, chaotic interior |

**Splitting:**
| Hook | Visual Direction |
|------|------------------|
| POV: they used the wrong tone and now you're rewriting your entire relationship | Baroque dramatic contrast |
| POV: you went from "I love them" to "I never want to see them again" in 10 minutes | Two opposing portraits |
| POV: you know you're splitting but you can't stop | Figure caught between two worlds |
| POV: the rational part of your brain watching the emotional part take over | Dueling figures |

**Quiet BPD:**
| Hook | Visual Direction |
|------|------------------|
| POV: you're having a complete breakdown and no one around you notices | Serene exterior, hidden chaos |
| POV: someone asks "are you okay?" and you have to decide how honest to be | Two faces, public vs private |
| POV: you're masking so hard you don't even know what's real anymore | Pre-Raphaelite woman, hollow eyes |
| POV: your therapist is the only one who knows how bad it really is | Confessional scene |

**Fear of Abandonment:**
| Hook | Visual Direction |
|------|------------------|
| POV: you're pushing them away to see if they'll stay | Figure turning away while looking back |
| POV: they said "we need to talk" and you've already planned the funeral of the relationship | Romantic painting, impending storm |
| POV: you're testing them again even though you know you shouldn't | Woman at crossroads |
| POV: you finally let someone in and now you're terrified | Vulnerable figure in doorway |

**Identity Disturbance:**
| Hook | Visual Direction |
|------|------------------|
| POV: someone asks what your hobbies are and you genuinely don't know | Blank mirror, faceless figure |
| POV: you're trying to figure out which version of yourself is real | Multiple reflections, fragmented |
| POV: you realized you've been mirroring everyone you've ever been close to | Chameleon transformation |
| POV: you meet someone new and start becoming them without meaning to | Two merging figures |

## App Mention Strategy (POV Posts)

**Use:** Rarely (profile link does the work)
**Why:** POV posts are about the EXPERIENCE, not solutions. Adding app mention breaks immersion.

If you must mention: Do it in the description, not the slides.
**Description example:** "if this is you... my DBT app actually helped me with this 🤍"

---

# FORMAT 3: TIPS/HACKS

## Purpose
Actionable value content. Positions you as experienced AND helpful. Drives saves and follows. Natural app integration.

## Structure (7 slides)

| Slide | Function |
|-------|----------|
| 1 | **Hook** | Promise of practical value |
| 2-4 | **The tips** | DBT skills explained simply, one per slide |
| 5 | **Emotional landing** | Quick, human reinforcement of the struggle |
| 6 | **App Slide** | Connect core emotion + DBT-Mind (free) + specific result |
| 7 | **CTA** | Fixed text: "i track it all on DBT-Mind. it's free." |

## Hook Formulas

### "Things That Actually Help" (Highest value signal)
**Formula:** `"X things that actually help when [specific BPD moment]"`

The word "actually" does heavy lifting—implies you've tried everything else and THIS works.

**Examples:**
- "3 things that actually help when your FP hasn't texted back"
- "3 things that actually help when you're splitting"
- "3 things that actually help when you're imploding instead of exploding"
- "3 things that actually help when abandonment fear takes over"
- "3 things that actually help when you don't know who you are"

### "The Skill I Wish I Learned Sooner"
**Formula:** `"the skill I wish I learned sooner for [symptom]"`

Regret framing = emotional hook. Implies transformation.

**Examples:**
- "the skill I wish I learned sooner for FP obsession"
- "the skill I wish I learned sooner for black and white thinking"
- "the skill no one talks about for quiet BPD"
- "the skill that stopped me from pushing people away"
- "the skill I wish I learned sooner for identity confusion"

### "What To Do When"
**Formula:** `"what to do when [crisis moment] — from someone who's been there"`

Personal authority + practical value.

**Examples:**
- "what to do when your whole mood depends on one person"
- "what to do before you say something you'll regret"
- "what to do when you're falling apart and can't tell anyone"
- "what to do when you need reassurance but can't ask"
- "what to do when you catch yourself mirroring someone"

### "How I Actually Stop"
**Formula:** `"how I actually stop [symptom] mid-spiral"`

"Actually" + crisis intervention = high save rate.

**Examples:**
- "how I stop spiraling when my FP is distant"
- "how I actually stop mid-split before I ruin everything"
- "how I finally learned to validate my own emotions"
- "how I stop testing people to see if they'll leave"
- "how I'm slowly figuring out who I actually am"

## DBT Skills to Feature

### By Topic

**FP/Attachment:**
- TIPP (Temperature, Intense exercise, Paced breathing, Progressive relaxation)
- Opposite Action
- Check the Facts
- STOP skill
- Self-soothing

**Splitting:**
- Wise Mind
- Check the Facts
- Dialectical thinking (holding two truths)
- Mindfulness of Current Emotion
- Urge Surfing

**Quiet BPD:**
- Self-validation
- Emotion naming
- TIPP
- Building Mastery
- Radical Acceptance

**Fear of Abandonment:**
- Check the Facts
- DEAR MAN
- Opposite Action
- Distress Tolerance
- Building evidence

**Identity Disturbance:**
- Values clarification
- Building Mastery
- Mindful awareness
- Self-validation
- Accumulating Positives

## App Mention Strategy (Tips Posts)

**Use:** Every tips post (it's natural here)
**Placement:** Slide 5 (after delivering value)
**Tone:** "Here's how to practice what I just taught"

### Why This Works:
Tips content has built-in solution structure. The app mention answers the natural question: "okay but how do I actually DO this?"

### Slide 5 Templates:

**Generic (works for any topic):**
"the DBT app I use walks you through each of these step by step"

**Skill-specific:**
"the app I use has guided exercises for [skill name] — it's the only thing that made it stick"

**Crisis-focused:**
"my DBT app has this as a crisis skill you can pull up in the moment"

**Learning-focused:**
"the app actually explains WHY these work, not just what to do"

---

# IMAGE CREATION: HIGGSFIELD NANO BANANA PRO

## Visual Strategy Overview

**Why Edward Hopper Style:**
- **Cinematic Solitude:** Hopper's work perfectly captures the "alone in a room" feeling that defines much of the BPD experience.
- **Dramatic Lighting:** His use of harsh light and deep shadows (chiaroscuro) serves as a visual metaphor for splitting and emotional extremes.
- **Urban Realism:** The mid-century American aesthetic feels grounded and "real," making the emotional content feel more personal and less like a historical museum piece.
- **Geometric Isolation:** His compositions use windows and doorways to frame solitary figures, mirroring the feeling of being "trapped" in one's own mind.

## Painting Style Mapping

| BPD Topic | Art Style | Why It Works |
|-----------|-----------|--------------|
| **Splitting** | Edward Hopper - High Contrast | Strong angular shadows and stark lighting = perfect visual metaphor for black/white thinking |
| **Fear of Abandonment** | Edward Hopper - Thresholds | Figures at doorways or windows, looking out toward an empty horizon, dramatic longing and uncertainty |
| **Quiet BPD** | Edward Hopper - Interior Solitude | Figures in contemplation, hidden emotion beneath a still surface, the melancholy of being alone in a room |
| **Identity Disturbance** | Edward Hopper - Cinematic Ambiguity | Uncanny stillness, figures that feel like they're playing a role in a stage-like setting |
| **FP/Emotional Intensity** | Edward Hopper - Night Windows | Intimate focus on single figures in urban settings, capturing the solitude of obsession |

## Higgsfield Nano Banana Pro Prompts

### General Prompt Structure:

```
Edward Hopper style painting of [subject description], [emotional state], [lighting/atmosphere], [composition details using architectural lines], cinematic realism, dramatic shadows
```

### Prompts by Topic

#### Favorite Person (FP)

**Slide 1 (Hook):**
```
Edward Hopper style painting of a solitary woman in a dimly lit city apartment at night, seen through a window, staring at a phone in her hands, intense focus, dramatic artificial light from a streetlamp hitting the wall, long shadows
```

**Slide 2 (Waiting):**
```
Edward Hopper style scene of a woman sitting alone at a diner counter, late night, staring into her cup, empty stools around her, cinematic lighting from above, reflecting in the large window, profound solitude
```

**Slide 3 (Relief/Joy):**
```
Edward Hopper style painting of a woman's silhouette against a brightly lit window, morning sun casting a long rectangular beam across the floor, her posture showing a mix of relief and exhaustion
```

**Slide 4 (Spiral):**
```
Edward Hopper style composition of a woman in a sparsely furnished hotel room, staring at a closed door, geometric shadows on the wall, sense of waiting and obsessive anticipation
```

#### Splitting

**Slide 1 (Hook):**
```
Edward Hopper style painting of a woman sitting at a table, half her face in harsh white light, half in deep blue shadow, stark divide, cinematic contrast, clinical yet emotional atmosphere
```

**Slide 2 (The shift):**
```
Edward Hopper style scene of a woman looking out a window at a desolate street, stark midday sun creating aggressive shadows, her expression unreadable and cold, geometric architectural framing
```

**Slide 3 (Aftermath):**
```
Edward Hopper style painting of a woman alone in a dark hallway, a single bright light coming from a cracked door at the end, sense of internal conflict and binary choice
```

**Slide 4 (Recognition):**
```
Edward Hopper style scene of a woman looking at her own reflection in a darkened windowpane at night, the reflection appearing like a different person, mood of eerie self-confrontation
```

#### Quiet BPD

**Slide 1 (Hook):**
```
Edward Hopper style painting of a woman at a crowded lunch counter, but she is utterly alone in her own space, looking down, others are blurred or turned away, the isolation of being in a crowd
```

**Slide 2 (The mask):**
```
Edward Hopper style interior of a sun-drenched room, a woman sitting perfectly still in a chair, hands folded, looking at a patch of light on the floor, hidden internal turmoil beneath total stillness
```

**Slide 3 (Internal chaos):**
```
Edward Hopper style scene of a woman standing on a porch at dusk, the house behind her is glowing with warm light but she is facing the dark garden, the contrast between external safety and internal shadows
```

**Slide 4 (Alone with it):**
```
Edward Hopper style painting of a woman in an office late at night, solitary figure under a desk lamp, the rest of the building is dark, weight of an invisible burden in her posture
```

#### Fear of Abandonment

**Slide 1 (Hook):**
```
Edward Hopper style painting of a woman standing at an open door, looking out at an empty road, the threshold creating a strong vertical frame, posture of hesitation and longing
```

**Slide 2 (Testing):**
```
Edward Hopper style scene of two figures in a room, the woman is looking away toward a window while the man is in the background, a palpable sense of emotional distance and the fear of the space between them
```

**Slide 3 (The fear realized):**
```
Edward Hopper style painting of a woman on a train, looking out at a passing landscape, muted colors, sense of being in transit and the fear of leaving or being left behind
```

**Slide 4 (Pattern recognition):**
```
Edward Hopper style composition of a woman sitting on a park bench as the sun sets, long shadows stretching toward her, empty benches receding into the distance, profound stillness
```

#### Identity Disturbance

**Slide 1 (Hook):**
```
Edward Hopper style painting of a woman looking into a dressing room mirror, several other mirrors in the background showing different angles of her, but none feel like the 'real' one, stage-like lighting
```

**Slide 2 (The mirroring):**
```
Edward Hopper style scene of a woman in a cafe, copying the exact posture of a woman at the next table, the subtle uncanny feeling of mirroring, cinematic realism
```

**Slide 3 (The void):**
```
Edward Hopper style painting of a woman's shadow on a bare wall, the shadow looking more substantial than the figure herself, geometric light from an unseen window
```

**Slide 4 (Searching):**
```
Edward Hopper style composition of a woman standing before a row of identical houses, looking confused about which one is hers, urban anonymity and the search for self
```

### Prompts for Tips/Hacks Format

**Opening slide (establishing authority):**
```
Edward Hopper style painting of a woman sitting in a sun-filled room, morning light, peaceful but solitary contemplation, clear geometric lines, sense of quiet resolve
```

**Skill demonstration slides:**
```
Edward Hopper style close-up of hands resting on a wooden table, strong directional light casting a long shadow, intimate and grounded composition
```

**Closing slide (hope/resolution):**
```
Edward Hopper style painting of a woman looking toward a bright horizon from a porch, long evening shadows, expression of quiet determination and hope
```

## Animation Notes

Higgsfield Nano Banana Pro can add subtle movement to generated images:
- Hair/fabric gentle movement
- Breathing animation
- Slight emotional shifts in expression
- Environmental elements (candle flicker, fog movement)

**Keep animations subtle** — 3-5 second clips, loop-friendly, not distracting from text overlay.

---

# COMPLETE EXAMPLE: SPLITTING POST

## Relatable/Emotional Format

**Slide 1 (Hook):**
- Text: "nobody talks about how fast you can go from 'soulmate' to 'enemy' in your head"
- Image prompt: `Edward Hopper style painting of a woman sitting at a table, half her face in harsh white light, half in deep blue shadow, stark divide, cinematic contrast`

**Slide 2:**
- Text: "one wrong word and suddenly I'm rewriting our entire history"
- Image prompt: `Edward Hopper style scene of a woman looking out a window at a desolate street, stark midday sun creating aggressive shadows, her expression unreadable and cold`

**Slide 3:**
- Text: "3 years of good memories just... gone"
- Image prompt: `Edward Hopper style painting of a woman alone in a dark hallway, a single bright light coming from a cracked door at the end, sense of internal conflict`

**Slide 4:**
- Text: "and the worst part is knowing it's not rational but feeling it anyway"
- Image prompt: `Edward Hopper style scene of a woman looking at her own reflection in a darkened windowpane at night, the reflection appearing like a different person`

**Slide 5:**
- Text: "finding out this has a name and there's actually something you can do about it"
- Image prompt: `Edward Hopper style interior of a sun-drenched room, a woman sitting perfectly still in a chair, hands folded, looking at a patch of light on the floor`

**Slide 6 (App Slide):**
- Text: "when i track it in DBT-Mind (free) -- i can come back to myself faster"
- Image prompt: `Edward Hopper style painting of a woman sitting in a sun-filled room, morning light, peaceful but solitary contemplation, sense of quiet resolve`

**Slide 7 (CTA):**
- Text: "i track it all on DBT-Mind. it's free."
- Image prompt: `Edward Hopper style painting of a quiet sunlit interior, warm daylight, calm and hopeful atmosphere, no phone or screens`

---

# POSTING STRATEGY

## Frequency
- 1-2 posts per day
- Consistency > volume

## Content Calendar Template (Weekly)

| Day | Format | Topic |
|-----|--------|-------|
| Monday | Relatable | Splitting |
| Tuesday | Tips/Hacks | FP coping skills |
| Wednesday | Relatable | Quiet BPD |
| Thursday | POV | Fear of abandonment |
| Friday | Relatable | Identity |
| Saturday | Tips/Hacks | Emotional regulation |
| Sunday | Relatable (with app mention) | FP |

## Hashtag Strategy

**Core hashtags (always use):**
#bpd #bpdawareness #bpdtiktok #mentalhealth #mentalhealthmatters

**Topic-specific:**
- FP: #favoriteperson #bpdfp #attachmentstyle
- Splitting: #splitting #bpdsplitting #blackandwhitethinking
- Quiet BPD: #quietbpd #highfunctioningbpd
- DBT: #dbt #dbtskills #dialecticalbehaviortherapy

**Growth hashtags:**
#fyp #foryou #relatable #mentalhealthtiktok

## Comment Strategy

When people comment "what app?" on posts with the soft app hook:

**Pin this reply:**
"it's called DBT-Mind! it has all the skills + exercises 🤍"

This creates social proof (others see the question/answer) and feels organic (answering questions, not pushing).
