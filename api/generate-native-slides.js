...
{"slides": ["Slide 1: ...", "Slide 2: ...", ..., "Slide 7: ..."]}`,
                messages: [{
                    role: 'user',
...
2. Use that struggle to create a viral Hook for Slide 1 following the required format. Ensure it feels authentic to the community and uses a helpful, supportive tone with a slight gen-z touch.`
        ...
        try {
            ...
            // Fallback for plain text responses: "Slide 1: ..." lines.
            const lineMatches = cleanedText.match(/Slide\s*\d+\s*[:\-]\s*.+/gi) || [];
            if (lineMatches.length > 0) {
                slides = lineMatches.map(s => s.trim());
            }
        }

        if (!slides || !Array.isArray(slides) || slides.length === 0) {
            throw new Error('Could not parse slides from model output');
        }

        slides = slides.map((s, i) => {
            const text = String(s).trim().replace(/^Slide\s*\d+\s*[:\-]\s*/i, '');
